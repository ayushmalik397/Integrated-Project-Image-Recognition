    ...
    function(response) {
      ...
      if(model_name == "age_gender_ethnicity" || model_name == "face-v1.3" || model_name == "celeb-v1.3") {
      	...
      	for(let i = 0; i < people_array.length; i++) {
      		...
      		if(model_name == "age_gender_ethnicity") {
      			...
      			// print top 5 ages
      			for(let a = 0; a < 5; a++)
      				concept_names += '<li class="inelements">' + age_array[a].name + ': <i>' + age_array[a].value + '</i></li>'; 
      			...
      			for(let e = 0; e < 3; e++)
      				concept_names += '<li class="inelements">' + ethnic_array[e].name + ': <i>' + ethnic_array[e].value + '</i></li>';

      			// Gender Header
      			concept_names += '<b><span style="font-size:10px">Gender</span></b>'

      			// print gender
      			concept_names += '<li class="inelements">' + gender.name + ': <i>' + gender.value + '</i></li>';
      		}

      		// For faces just print bounding boxes
      		else if(model_name == "face-v1.3") {
      			// Top Row
      			concept_names += '<li class="inelements">Top Row: <i>' + people_array[i].region_info.bounding_box.top_row + '</i></li>';
      			concept_names += '<li class="inelements">Left Column: <i>' + people_array[i].region_info.bounding_box.left_col + '</i></li>';
      			concept_names += '<li class="inelements">Bottom Row: <i>' + people_array[i].region_info.bounding_box.bottom_row + '</i></li>';
      			concept_names += '<li class="inelements">Right Column: <i>' + people_array[i].region_info.bounding_box.right_col + '</i></li>';
      		}
      		...
      		// Celebrity
      			...
      			for(var c=0; c < 10; c++)
      				concept_names += '<li class="inelements">' + tag_array[c].name + ': <i>' + tag_array[c].value + '</i></li>';
      		}
      ...
      // Check for color model since it has its own unique JSON
        ...
        for (let i = 0; i < tag_array.length; i++)
          concept_names += '<li class="inelements">' + tag_array[i].name + ': <i>' + tag_array[i].value + '</i></li>';
      ...
      // Color Model
        ...
        for (let col = 0; col < tag_array.length; col++)
          concept_names += '<li class="inelements">' + tag_array[col].w3c.name + ': <i>' + tag_array[col].value + '</i></li>';
      ...
      concept_names = '<ul class="elements" style="margin-right:20px; margin-top:20px; columns:' + column_count + '; -webkit-columns:' + column_count + '; -moz-columns:' + column_count + ';">' + concept_names;
